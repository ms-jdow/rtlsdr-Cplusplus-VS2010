...
class e4kTuner : public ITuner
{
public:
	e4kTuner( rtlsdr* in_dev );
	~e4kTuner() {}

	//	ITuner interface
	virtual int init					( rtlsdr* base );	//	new?
	virtual int exit					( void );	//	delete?
...
protected:
	...
	int			e4k_reg_read			( uint8_t reg );
	int			e4k_reg_set_mask		( uint8_t reg
										, uint8_t mask
										, uint8_t val
										);
	int			e4k_field_write			( const struct reg_field *field
										, uint8_t val
										);
	int			e4k_field_read			( const struct reg_field *field );
	int			closest_arr_idx			( const uint32_t *arr
	...
	int			find_if_bw				( enum e4k_if_filter filter
										, uint32_t bw
										);
	int			is_fvco_valid			( uint32_t fvco_z );
	int			is_fosc_valid			( uint32_t fosc );
	int			is_z_valid				( uint32_t z );
	int			use_3ph_mixing			( uint32_t flo );
	uint64_t	compute_fvco			( uint32_t f_osc
										, uint8_t z
										, uint16_t x
										);
	uint32_t	compute_flo				( uint32_t f_osc
										, uint8_t z
										, uint16_t x
										, uint8_t r
										);
	...
	int			e4k_tune_params			( struct e4k_pll_params *p );
	uint32_t	e4k_compute_pll_params	( struct e4k_pll_params *oscp
										, uint32_t fosc
										, uint32_t intended_flo
										);
	int			e4k_if_filter_bw_get	( enum e4k_if_filter filter );
	int			e4k_if_filter_bw_set	( enum e4k_if_filter filter
										, uint32_t bandwidth
										);
	int			e4k_if_filter_chan_enable
										( int on );
	int			e4k_rf_filter_set		( void );
	
	int			e4k_manual_dc_offset	( int8_t iofs
										, int8_t irange
										, int8_t qofs
										, int8_t qrange
										);
