...
fc0012Tuner::~fc0012Tuner()
{
}

int fc0012Tuner::init( rtlsdr* base )
{
	if ( base == NULL )
		return -1;
	rtldev = base;
...
int fc0012Tuner::set_freq( uint32_t freq /* Hz */, uint32_t *lo_freq_out )
{
	if ( lo_freq_out )
		*lo_freq_out = freq;
...
int fc0012Tuner::fc0012_writereg( uint8_t reg, uint8_t val )
{
	uint8_t data[ 2 ];
	data[ 0 ] = reg;
	data[ 1 ] = val;

	if ( rtldev->rtlsdr_i2c_write_fn( FC0012_I2C_ADDR, data, 2 ) < 0)
		return -1;

	return 0;
}

int fc0012Tuner::fc0012_readreg( uint8_t reg, uint8_t *val )
{
	uint8_t data = reg;

	if ( rtldev->rtlsdr_i2c_write_fn( FC0012_I2C_ADDR, &data, 1 ) < 0)
		return -1;

	if ( rtldev->rtlsdr_i2c_read_fn( FC0012_I2C_ADDR, &data, 1 ) < 0)
		return -1;
...
int fc0012Tuner::fc0012_set_params( uint32_t freq
								  , uint32_t bandwidth
								  )
{
	int i;
	...
	if ( freq < 55625000 )				/* freq * 64 < 3560000000 */
	{
		multi    = 64;
		reg[ 5 ] = 0x82;
		reg[  6] = 0x02;
	...
	if ( freq < 74167000 )				/* freq * 48 < 3560000000 */
	{
		multi    = 48;
		reg[ 5 ] = 0x42;
		reg[ 6 ] = 0x00;
	...
	if ( freq < 111250000 )				/* freq * 32 < 3560000000 */
	{
		multi    = 32;
		reg[ 5 ] = 0x42;
		reg[ 6 ] = 0x02;
	...
	if ( freq < 148334000 )			/* freq * 24 < 3560000000 */
	{
		multi    = 24;
		reg[ 5 ] = 0x22;
		reg[ 6 ] = 0x00;
	...
	if ( freq < 222500000 )				/* freq * 16 < 3560000000 */
	{
		multi    = 16;
		reg[ 5 ] = 0x22;
		reg[ 6 ] = 0x02;
	...
	if ( freq < 296667000 )				/* freq * 12 < 3560000000 */
	{
		multi    = 12;
		reg[ 5 ] = 0x12;
		reg[ 6 ] = 0x00;
	...
	if ( freq < 445000000 )				/* freq * 8 < 3560000000 */
	{
		multi    = 8;
		reg[ 5 ] = 0x12;
		reg[ 6 ] = 0x02;
	...
	if ( freq < 593334000 )				/* freq * 6 < 3560000000 */
	{
		multi    = 6;
		reg[ 5 ] = 0x0a;
		reg[ 6 ] = 0x00;
	...
	/* VCO Calibration */
	ret = fc0012_writereg(  0x0e, 0x80 );
	if ( !ret )
		ret = fc0012_writereg( 0x0e, 0x00 );
...
//		msleep(10);
	...
	if ( vco_select )
	{
		...
		{
			reg[ 6 ] &= ~0x08;
			ret = fc0012_writereg( 0x06, reg[ 6 ]);
			if ( !ret )
				ret = fc0012_writereg( 0x0e, 0x80 );
			if ( !ret )
				ret = fc0012_writereg( 0x0e, 0x00 );
...
int fc0012Tuner::fc0012_set_gain( int gain )
{
	int ret;
	uint8_t tmp = 0;

	ret = fc0012_readreg(  0x13, &tmp );
