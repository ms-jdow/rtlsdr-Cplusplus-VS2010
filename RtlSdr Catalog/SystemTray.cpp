...
#ifdef IS_SMA
const UINT CSystemTray::m_nTimerID    = 4562;
#else
...
CInvisibleWnd  CSystemTray::m_wndInvisible;
BOOL  CSystemTray::m_bWindowHidden;	// window is hidden
...
void CSystemTray::Initialise()
{
    ...
    GetVersionEx( &os );
    m_bWin2K = ( VER_PLATFORM_WIN32_NT == os.dwPlatformId
									   && os.dwMajorVersion >= 5 );
...
BOOL CSystemTray::Create( CWnd* pParent
...
{
	TRACE ( "Tray popup Create hidden = %d\n", bHidden );

    // this is only for Windows 95 (or higher)
    m_bEnabled = ( GetVersion() & 0xff ) >= 4;
    if ( !m_bEnabled ) 
    {
		TRACE ( "Version number too low!\n" );
        ASSERT( FALSE );
        return FALSE;
    }
    ...
    m_tnd.hWnd   = pParent && pParent->GetSafeHwnd()
				 ? pParent->GetSafeHwnd()
				 : m_hWnd;
    m_tnd.uID    = uID;
    m_tnd.hIcon  = icon;
    ...
    if ( m_bWin2K && szBalloonTip )
    {
        ...
        ASSERT( NIIF_NONE == dwBalloonIcon
			 || NIIF_INFO == dwBalloonIcon
			 || NIIF_WARNING == dwBalloonIcon
			 || NIIF_ERROR == dwBalloonIcon
			  );
        ...
        m_tnd.dwInfoFlags = dwBalloonIcon | NIIF_NOSOUND;	// don't dink
    }
	...
	TRACE ( "Adding shell notify icon\n" );
    ...
    {
		//	In case running without a shell compensate for it.
		while(( tnow = GetTickCount()) - tstart < 60000 )
		{
			TRACE( "Trying to add shell notify icon\n" );

			bResult = Shell_NotifyIcon( NIM_ADD, &m_tnd );
			err		= GetLastError();

			TRACE( "Shell notify icon result = %d in %dms, err = %d\n"
				 ,  bResult, tnow-tstart, err );
	...
	msg.Format( _T( "D: Tray icon creation took %d ms with result %d error %d" )
			  , GetTickCount() - tstart
			  , bResult
			  , err
			  );
							 ...
							 )
{
	...
	m_tnd.uFlags = NIF_INFO | NIIF_NOSOUND;	// don't beep!
	_tcsncpy_s( m_tnd.szInfo, szText, 256 );
	if ( szTitle )
		_tcsncpy_s( m_tnd.szInfoTitle, szTitle, 64 );
	else
		m_tnd.szInfoTitle[ 0 ] = _T( '\0' );
	m_tnd.dwInfoFlags = dwIcon | NIIF_NOSOUND;	// don't dink
...
BEGIN_MESSAGE_MAP(CSystemTray, CWnd)
	...
	ON_COMMAND(ID_SHOW_MAIN_WND, OnShowMainWnd)
	ON_UPDATE_COMMAND_UI(ID_SHOW_MAIN_WND, OnUpdateShowMainWnd)
	ON_COMMAND(ID_HIDE_MAIN_WND, OnHideMainWnd)
	ON_UPDATE_COMMAND_UI(ID_HIDE_MAIN_WND, OnUpdateHideMainWnd)
	ON_COMMAND(ID_TRUE_EXIT, OnAppExit)
...
void CSystemTray::OnTimer( UINT_PTR nIDEvent ) 
{
    ...
    if (( m_nAnimationPeriod > 0 )
	&&	( m_nAnimationPeriod < period.GetTotalSeconds()))
    {
        StopAnimation();
        return;
    }
...
LRESULT CSystemTray::OnTrayNotification( WPARAM wParam, LPARAM lParam )
{
    ...
    {    
		...
		//	If we are running the tray handler on a separate WinThread
		//	rather than the main window thread, it looks like we have 
		//	to use our own window handle and not the main window handle
		//	to bring up the popup menu. This has the unfortunate side
		//	effect that the messages will come back to us and not to 
		//	the main window. So we have to have message handlers for 
		//	all of the menu items, and forward them on to AfxGetMainWnd.

        if ( !::TrackPopupMenu( pSubMenu->m_hMenu
							  , 0
							  , pos.x
							  , pos.y
							  , 0
							  ...
							  ))
		{
			...
			TRACE ( "TrackPopupMenu failed with error %d\n", err );
		}
...
LRESULT CSystemTray::WindowProc( UINT message
							   , WPARAM wParam
							   , LPARAM lParam
							   )
{
...
void CSystemTray::InstallIconPending( void )
{
	...
	//	This assert seems to fail with fast user switching when the screen
	//	saver has a password.  As we log into the fast user switching page
	//	after the screen saver timeout kicks in, we get hit with this assert.
...
void CSystemTray::MaximiseFromTray( CWnd* pWnd, BOOL bForceAnimation /*= TRUE*/)
{
	...
	TRACE ( "Redrawing the main window\n" );
...
///// RtlSdr Catalog
//
void CSystemTray::OnShowMainWnd( void )
{
	CWnd* pMainWnd = AfxGetMainWnd();
	if ( pMainWnd != NULL )
		pMainWnd->PostMessage( WM_COMMAND, ID_SHOW_MAIN_WND );
}

void CSystemTray::OnHideMainWnd( void )
{
	CWnd* pMainWnd = AfxGetMainWnd();
	if ( pMainWnd != NULL )
		pMainWnd->PostMessage( WM_COMMAND, ID_HIDE_MAIN_WND );
}

void CSystemTray::OnUpdateShowMainWnd( CCmdUI* pCmdUI )
{
    pCmdUI->Enable( IsIconic() || WindowHidden());
}

void CSystemTray::OnUpdateHideMainWnd( CCmdUI* pCmdUI )
{
    pCmdUI->Enable( IsIconic() || !WindowHidden());
}

void CSystemTray::OnAppExit() 
{
	CWnd* pMainWnd = AfxGetMainWnd();
	if ( pMainWnd != NULL )
		pMainWnd->PostMessage( WM_COMMAND, ID_TRUE_EXIT );
}

void CSystemTray::OnAppAbout() 
{
	CWnd* pMainWnd = AfxGetMainWnd();
	if ( pMainWnd != NULL )
		pMainWnd->PostMessage( WM_COMMAND, ID_APP_ABOUT );
}
